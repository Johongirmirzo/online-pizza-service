...
const pizzaSizeCreateData = object({
  ...
  photo: string().required('Pizza photo is required!'),
...
const createPizzaData = object({
  ...
  vegan: boolean().required('Vegan value is required!'),
  spiceLevel: string()
...
const pizzaSizeEditData = object({
  size: string().matches(
    ...
    'Please choose one of the pizza sizes'
  ),
  price: number()
    .min(5000, 'Minimum price must be 5000')
    .max(1000000, 'Maximum price must be 1000000'),
...
const editPizzaData = object({
  name: string()
    .min(5, 'Minimum pizza name length must be 5')
    .max(35, 'Maximum pizza name length must be 35'),
  vegan: boolean().typeError('Please choose one of choices'),
  spiceLevel: string().matches(
    /not_spicy|mild|hot/i,
    'Please choose one of the spice options'
  ),
  categoryId: string().required('Category is required!'),
  pizzaSizes: array().of(pizzaSizeEditData),
})

export { createPizzaData, editPizzaData }
