<template>
  <section class="applicants">
    <header class="applicants__header">
      <h1 class="applicants__title">Accepted Applicants</h1>
    </header>
    <DeleteModal
    ...
    />
    <PizzaLoader v-if="isLoadingApplicants" />
...
const downloadCSVFilename = "accepted-applicants.csv";
...
const handleDownloadExcelClick = () => {
  generateExcel(applicantsPdfTable.value, "accepted-applicants", "applicants");
};
const handleDownloadPdfClick = () => {
  generatePDF("accepted-applicants-table", applicantsPdfTable.value);
};
...
const fetchAllApplicants = async () => {
  ...
  applicants.value = [
    ...applicantsResp.data.data.filter(
      (applicant: IApplicant) => applicant.applicationStatus === "ACCEPTED"
    ),
  ];
...
const toggleApplicantStatus = async (
  ...
  try {
    ...
    status === "REJECTED"
      ? navigateTo("/applicant/rejected-applicants")
