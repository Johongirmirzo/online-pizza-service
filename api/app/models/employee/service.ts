...
const EmployeeService = {
  ...
  async createEmployee(data: IEmployee, file: any) {
    try {
      ...
      if (!employee) {
        ...
        const employeePhoto = await cloudinary.v2.uploader.upload(
          file.path,
          options
        )
  ...
  async editEmployee(id: string, employeeData: any, file: any) {
    try {
      const employee = await prisma.employee.findUnique({ where: { id } })
      ...
      if (!employee) {
        if (file) {
          fs.rmSync(
            path.join(__dirname, `../../assets/images/${file.fileName}`)
          )
        }
        ...
        if (employeeWithGivenData) {
          if (
            employee.id !== employeeWithGivenData.id &&
        ...
        const employeeParsedData = {
          ...
          salary: employeeData?.salary
            ...
            : employee.salary,
          emergancyContacts: employeeData?.emergancyContacts
            ? JSON.parse(employeeData?.emergancyContacts)
        ...
        if (file) {
          ...
          const newPhoto = await cloudinary.v2.uploader.upload(
            file.path,
            options
          )
  ...
  async getAllEmergencyContacts(id: string) {
    try {
      const employee = await prisma.employee.findUnique({ where: { id } })
      if (!employee) {
        return {
  ...
  async createEmergencyContact(id: string, emergencyData: IEmergencyContact) {
    try {
      ...
      if (!employee) {
        ...
        if (employee.emergancyContacts.length > 3) {
          ...
          return {
            newEmergencyContact:
              newEmergencyContact.emergancyContacts[
                newEmergencyContact.emergancyContacts.length - 1
              ],
            statusCode: 201,
            error: '',
          }
  ...
  async editEmergencyContact(
    ...
    emergencyData: IEmergencyContact
  ) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      ...
      if (!employee) {
        ...
        employee.emergancyContacts = employee.emergancyContacts.map((ec: any) =>
          ec.id === emergencyContactId ? { ...ec, ...emergencyData } : ec
        )
        await prisma.employee.update({
          where: { id: employeeId },
          data: {
  ...
  async deleteEmergencyContact(employeeId: string, emergencyContactId: string) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
        select: {
      ...
      if (!employee) {
        ...
        const emergencyContactIndex = employee.emergancyContacts.findIndex(
          (e: any) => e.id === emergencyContactId
        )
  ...
  async getEmployeeNote(employeeId: string) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
  ...
  async createEmployeeNote(employeeId: string, employeeNoteData: any) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
        select: {
        ...
        const newEmployeeNote = await prisma.employee.update({
          ...
          data: {
            employeeNotes: [
              ...
              { id: uuid(), ...employeeNoteData },
            ],
          },
        })
        return {
          newEmployeeNote:
            newEmployeeNote.employeeNotes[
              newEmployeeNote.employeeNotes.length - 1
            ],
          statusCode: 201,
          error: '',
        }
  ...
  async editEmployeeNote(
    noteId: string,
    employeeId: string,
    employeeNoteData: any
  ) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      ...
      if (!employee) {
        ...
        employee.employeeNotes = employee.employeeNotes.map((ec: any) =>
          ec.id === noteId ? { ...ec, ...employeeNoteData } : ec
        )
        await prisma.employee.update({
          where: { id: employeeId },
          data: {
  ...
  async deleteEmployeeNote(noteId: string, employeeId: string) {
    try {
      ...
      if (!employee) {
        ...
        const emergencyContactIndex = employee?.employeeNotes.findIndex(
          (e: any) => e.id === noteId
        )
        if (emergencyContactIndex < 0) {
          return {
  ...
  async getEmployeeLeaveDay(leaveDayId: string, employeeId: string) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      ...
      if (!employee) {
        ...
        const leaveDay = employee.leaveDays.find(
          (leaveDay: any) => leaveDay.id === leaveDayId
        )
  ...
  async createEmployeeLeaveDay(employeeId: string, leaveDayData: any) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      })
      if (!employee) {
        ...
        const newEmployeeNote = await prisma.employee.update({
          ...
          data: {
            leaveDays: [
              ...employee.leaveDays,
              { id: uuid(), ...leaveDayData, status: 'NEW' },
            ],
          },
        })
        return {
          newEmployeeLeaveDay:
            newEmployeeNote.leaveDays[newEmployeeNote.leaveDays.length - 1],
          statusCode: 201,
          error: '',
        }
      }
  ...
  async editEmployeeLeaveDay(
    employeeLeaveDayId: string,
    employeeId: string,
    employeeLeaveDayData: any
  ) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      ...
      if (!employee) {
        ...
        employee.leaveDays = employee.leaveDays.map((ec: any) =>
          ec.id === employeeLeaveDayId ? { ...ec, ...employeeLeaveDayData } : ec
        )
        await prisma.employee.update({
          where: { id: employeeId },
          data: {
  ...
  async changeEmployeeLeaveDayStatus(
    leaveDayId: string,
    employeeId: string,
    status: string
  ) {
    try {
      ...
      if (!employee) {
        ...
        employee.leaveDays = employee.leaveDays.map((leaveDay: any) =>
          leaveDay.id === leaveDayId ? { ...leaveDay, status } : leaveDay
        )
        await prisma.employee.update({
          where: { id: employeeId },
          data: {
  ...
  async deleteEmployeeLeaveDay(leaveDayId: string, employeeId: string) {
    try {
      const employee = await prisma.employee.findUnique({
        where: { id: employeeId },
      })
      if (!employee) {
        ...
        const leaveDayIndex = employee.leaveDays.findIndex(
          (leaveDay: any) => leaveDay.id === leaveDayId
        )
