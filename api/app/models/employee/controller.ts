...
const EmployeeController = {
    ...
    async getEmployee(req: Request, res: Response){
        ...
        const {error, statusCode, employee} = await EmployeeService.getEmployee(id);
    ...
    async createEmployee(req: Request, res: Response){
        const {newEmployee, error, statusCode} = await EmployeeService.createEmployee(req.body, req.file);
        if(error){
            return res.status(statusCode).json({errors: error})
        } else {
    ...
    async editEmployee(req: Request, res: Response){
        const id = Number(req.params.id);
        const {error, statusCode} = await EmployeeService.editEmployee(id, req.body, req.file);
    ...
    async getAllEmergencyContacts(req: Request, res: Response){
        const id = Number(req.params.id);
        const {emergencyContacts, statusCode, error} = await EmployeeService.getAllEmergencyContacts(id)
    ...
    async createEmergencyContact(req: Request, res: Response){
        const id = Number(req.params.id);
        const {newEmergencyContact, statusCode, error} = await EmployeeService.createEmergencyContact(id, req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async editEmergencyContact(req: Request, res: Response){
        ...
        const {statusCode, error} = await EmployeeService.editEmergencyContact(Number(id), emergencyContactId, req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async deleteEmergencyContact(req: Request, res: Response){
        const {id, emergencyContactId} = req.params;
        const {statusCode, error} = await EmployeeService.deleteEmergencyContact(Number(id), emergencyContactId)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
            return res.status(statusCode).end()
    ...
    async getEmployeeNote(req: Request, res: Response){
        const id = Number(req.params.id);
        const {employeeNote, statusCode, error} = await EmployeeService.getEmployeeNote(id)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async createEmployeeNote(req: Request, res: Response){
        const id = Number(req.params.id);
        const {newEmployeeNote, statusCode, error} = await EmployeeService.createEmployeeNote(id, req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async editEmployeeNote(req: Request, res: Response){
        ...
        const {statusCode, error} = await EmployeeService.editEmployeeNote(noteId, Number(employeeId), req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
            return res.status(statusCode).json({message: "Emergency contact is updated successfully!"})
    ...
    async deleteEmployeeNote(req: Request, res: Response){
        const {noteId, employeeId} = req.params;
        const {statusCode, error} = await EmployeeService.deleteEmployeeNote(noteId, Number(employeeId))
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
            return res.status(statusCode).end()
    ...
    async getEmployeeLeaveDay(req: Request, res: Response){
        ...
        const {employeeLeaveDays, statusCode, error} = await EmployeeService.getEmployeeLeaveDay(leaveDayId, Number(employeeId));
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async createEmployeeLeaveDay(req: Request, res: Response){
        const id = Number(req.params.id);
        const {newEmployeeLeaveDay, statusCode, error} = await EmployeeService.createEmployeeLeaveDay(id, req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async changeEmployeeLeaveDayStatus(req: Request, res: Response){
        const {leaveDayId, employeeId} = req.params;
        const {statusCode, error} = await EmployeeService.changeEmployeeLeaveDayStatus(leaveDayId, Number(employeeId), req.body.status)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async editEmployeeLeaveDay(req: Request, res: Response){
        ...
        const {statusCode, error} = await EmployeeService.editEmployeeLeaveDay(leaveDayId, Number(employeeId), req.body)
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
    ...
    async deleteEmployeeLeaveDay(req: Request, res: Response){
        const {leaveDayId, employeeId} = req.params;
        console.log({leaveDayId, employeeId})
        const {statusCode, error} = await EmployeeService.deleteEmployeeLeaveDay(leaveDayId, Number(employeeId))
        if(statusCode === 400 || statusCode === 404){
            return res.status(statusCode).json({errors: error}) 
        } else {
            return res.status(statusCode).end()
