...
const CustomerController = {
  ...
  async getCustomer(req: Request, res: Response) {
    ...
    const { customer, statusCode, error } = await CustomerService.getCustomer(
      id
    )
  ...
  async loginCustomer(req: Request, res: Response) {
    try {
      const { customer, statusCode, error } =
      ...
      if (statusCode === 400 && !customer) {
        ...
        if (customer) {
          ...
          const accessToken = generateToken<ICustomerToken>(
            { id, phone, status: CustomerStatus[status] },
            `${ACCESS_TOKEN_PRIVATE_KEY}`,
            `${ACCESS_TOKEN_EXPIRATION_TIME}`
          )
          const refreshToken = generateToken<ICustomerToken>(
            { id, phone, status: CustomerStatus[status] },
            `${REFRESH_TOKEN_PRIVATE_KEY}`,
            `${REFRESH_TOKEN_EXPIRATION_TIME}`
          )
  ...
  async registerCustomer(req: Request, res: Response) {
    const { newCustomer, statusCode, error } =
  ...
  async updateCustomer(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await CustomerService.updateCustomer(
      id,
      req.body
    )
    if (statusCode === 400 || statusCode === 404) {
  ...
  async changeCustomerStatus(req: Request, res: Response) {
    ...
    const { statusCode, error } = await CustomerService.changeCustomerStatus(
      id,
      status
    )
    if (statusCode === 400 || statusCode === 404) {
  ...
  async createCustomerAddress(req: Request, res: Response) {
    const id = req.params.id
    const { newAddress, statusCode, error } =
      await CustomerService.createCustomerAddress(id, req.body)
    if (statusCode === 400) {
      return res.status(statusCode).json({ errors: error })
    } else {
  ...
  async updateCustomerAddress(req: Request, res: Response) {
    ...
    const { statusCode, error } = await CustomerService.updateCustomerAddress(
      addressId,
      customerId,
      req.body
    )
  ...
  async setDefaultCustomerAddress(req: Request, res: Response) {
    ...
    const { statusCode, error } =
      await CustomerService.setDefaultCustomerAddress(addressId, customerId)
    if (statusCode === 400 || statusCode === 404) {
      return res.status(statusCode).json({ errors: error })
    } else {
      return res
  ...
  async deleteCustomerAddress(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await CustomerService.deleteCustomerAddress(
      id
    )
    if (statusCode === 400 || statusCode === 404) {
      return res.status(statusCode).json({ errors: error })
