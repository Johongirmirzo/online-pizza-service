...
const ApplicantController = {
    ...
    async getApplicant(req: Request, res: Response){
        ...
        const {applicant, statusCode, error} = await ApplicantService.getApplicant(id); 
    ...
    async createApplicant(req: Request, res: Response){
        const {newApplicant, statusCode, error} = await ApplicantService.createApplicant(req.body, req.files)
        if(!newApplicant){
            return res.status(statusCode).json({errors: error})
        }
    ...
    async changeApplicantStatus(req: Request, res: Response){
        ...
        const {error, statusCode} = await ApplicantService.changeApplicantStatus(id, status)
        if(statusCode === 404){
            return res.status(statusCode).json({errors: error})
        } 
