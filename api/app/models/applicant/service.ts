...
const ApplicantService = {
  ...
  async createApplicant(data: any, files: any) {
    try {
      ...
      const applicationData = Object.entries(data)
        ...
        .reduce((obj, el) => {
          ...
          return {
            ...obj,
            [el[0]]:
              el[0] == 'hasBankAccount' ? JSON.parse(encodedData) : el[1],
          }
        }, {}) as IApplicant

      const applicantPhoto = await cloudinary.v2.uploader.upload(
        files.applicantPhoto[0].path,
        options
      )
      const passportPhoto = await cloudinary.v2.uploader.upload(
        files.passportPhoto[0].path,
        options
      )
      const resume = await cloudinary.v2.uploader.upload(
        files.resume[0].path,
        options
      )
      switch (data.role) {
        ...
        case 'MANAGER':
        case 'AREA_COACH':
          const panPhoto = await cloudinary.v2.uploader.upload(
      ...
      fs.rm(
        path.join(
          __dirname,
          `../../assets/images/${files.applicantPhoto[0].path}`
        ),
        () => {}
      )
      fs.rm(
        path.join(
          __dirname,
          `../../assets/images/${files.passportPhoto[0].path}`
        ),
        () => {}
      )
      fs.rm(
        path.join(__dirname, `../../assets/images/${files.resume[0].path}`),
        () => {}
      )
      return { newApplicant: null, statusCode: 400, error: 'Bad Request' }
    }
  ...
  async changeApplicantStatus(id: string, status: ApplicantStatus) {
