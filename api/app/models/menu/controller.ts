...
const MenuController = {
  ...
  async createDip(req: Request, res: Response) {
    const { newDip, statusCode, error } = await MenuService.createDip(
      req.body,
      req.file
    )
  ...
  async editDip(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.editDip(
      id,
      req.body,
      req.file
    )
  ...
  async changeDipStatus(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.changeDipStatus(
      id,
      req.body.status
    )
    if (statusCode === 404 || statusCode === 400) {
  ...
  async createMenuItem(req: Request, res: Response) {
    const { newMenuItem, statusCode, error } = await MenuService.createMenuItem(
      req.body,
      req.files
    )
    if (statusCode === 404 || statusCode === 400 || statusCode === 409) {
  ...
  async editMenuItem(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.editMenuItem(
      id,
      req.body,
      req.files
    )
  ...
  async changeMenuItemStatus(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.changeMenuItemStatus(
      id,
      req.body.status
    )
    if (statusCode === 400 || statusCode === 404) {
  ...
  async createPizza(req: Request, res: Response) {
    const { newPizza, statusCode, error } = await MenuService.createPizza(
      req.body,
      req.files
    )
    if (statusCode === 404 || statusCode === 400 || statusCode === 409) {
  ...
  async editPizza(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.editPizza(
      id,
      req.body,
      req.files
    )
  ...
  async changePizzaStatus(req: Request, res: Response) {
    const id = req.params.id
    const { statusCode, error } = await MenuService.changePizzaStatus(
      id,
      req.body.status
    )
    if (statusCode === 404 || statusCode === 400) {
