...
const MenuService = {
  ...
  async createDip(dipData: IDip, dipPhotoFile: any) {
    try {
      ...
      if (dip) {
        if (dipPhotoFile) {
          fs.rm(
            path.join(
              __dirname,
              `../../assets/images/${dipPhotoFile.filename}`
            ),
            () => {}
          )
        }
        ...
        const dipPhoto = await cloudinary.v2.uploader.upload(
          dipPhotoFile.path,
          options
        )
        const newDip = await prisma.dip.create({
          data: {
            ...
            categoryId: dipData.categoryId.toString(),
      ...
      if (dipPhotoFile) {
        fs.rm(
          path.join(__dirname, `../../assets/images/${dipPhotoFile.filename}`),
          () => {}
        )
      }
  ...
  async editDip(id: string, data: IDip, dipPhotoFile: any) {
    try {
      let dipData = {}
      const dipWithGivenData = await prisma.dip.findUnique({
  ...
  async changeDipStatus(id: string, status: Status) {
    try {
      const dip = await prisma.dip.findUnique({ where: { id } })
      if (!dip) {
  ...
  async createMenuItem(data: IMenuItem, menuItemPhotos: any) {
    const menuItemData = Object.entries(data).reduce(
      (obj, el) =>
        /pieces/i.test(el[0])
          ? { ...obj, [el[0]]: JSON.parse(el[1]) }
          : { ...obj, [el[0]]: el[1] },
    ...
    const removePhoto = () => {
      if (menuItemPhotos.small) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${menuItemPhotos.small[0].filename}`
          )
        )
      }
      if (menuItemPhotos.medium) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${menuItemPhotos.medium[0].filename}`
          )
        )
      }
      if (menuItemPhotos.large) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${menuItemPhotos.large[0].filename}`
          )
        )
      }
    }
    ...
    try {
      ...
      if (menuItem) {
        ...
        if (menuItemPhotos.small) {
          smallPhoto = await cloudinary.v2.uploader.upload(
            menuItemPhotos.small[0].path,
            options
          )
        }
        if (menuItemPhotos.medium) {
          mediumPhoto = await cloudinary.v2.uploader.upload(
            menuItemPhotos.medium[0].path,
            options
          )
        }
        if (menuItemPhotos.large) {
          largePhoto = await cloudinary.v2.uploader.upload(
            menuItemPhotos.large[0].path,
            options
          )
        }
  ...
  async editMenuItem(menuItemId: string, data: IMenuItem, menuItemPhotos: any) {
    const menuItemData = Object.entries(data).reduce(
      (obj, el) =>
        /pieces/i.test(el[0])
          ? { ...obj, [el[0]]: JSON.parse(el[1]) }
    ...
    try {
      const menuItemWithGivenData = await prisma.menuItem.findUnique({
        where: { name: data.name },
      })
      const menuItem = await prisma.menuItem.findUnique({
        where: { id: menuItemId },
      ...
      if (!menuItem) {
        ...
        if (menuItemWithGivenData) {
          if (
            menuItem.id !== menuItemWithGivenData.id &&
        ...
        if (
          menuItemParsedPieces.length > menuItemData.pieces.length &&
          ...
          for (let i = 0; i < menuItemParsedPieces.length; i++) {
            ...
            for (let j = 0; j < menuItemData.pieces.length; j++) {
              if (
                menuItemParsedPieces[i].photo_id ===
                menuItemData.pieces[j]?.photo_id
              ) {
                isThereMatch = true
                break
              }
  ...
  async changeMenuItemStatus(menuItemId: string, status: Status) {
    try {
      const menuItem = await prisma.menuItem.findUnique({
        where: { id: menuItemId },
      })
  ...
  async createPizza(pizzaData: any, pizzaPhotoFiles: any) {
    const removePhoto = () => {
      if (pizzaPhotoFiles.small) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${pizzaPhotoFiles.small[0].filename}`
          )
        )
      }
      if (pizzaPhotoFiles.medium) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${pizzaPhotoFiles.medium[0].filename}`
          )
        )
      }
      if (pizzaPhotoFiles.large) {
        fs.rmSync(
          path.join(
            __dirname,
            `../../assets/images/${pizzaPhotoFiles.large[0].filename}`
          )
        )
      }
    }
    ...
    try {
      ...
      if (pizza) {
        ...
        const parsedPizzaSizes = JSON.parse(pizzaData.pizzaSizes).map(
          (pizzaSize: any) => ({
            ...pizzaSize,
            price: pizzaSize.price ? pizzaSize.price : '',
            weight: pizzaSize.weight ? pizzaSize.weight : '',
          })
        )
        ...
        if (pizzaPhotoFiles.small) {
          smallPhoto = await cloudinary.v2.uploader.upload(
            pizzaPhotoFiles.small[0].path,
            options
          )
        }
        if (pizzaPhotoFiles.medium) {
          mediumPhoto = await cloudinary.v2.uploader.upload(
            pizzaPhotoFiles.medium[0].path,
            options
          )
        }
        if (pizzaPhotoFiles.large) {
          largePhoto = await cloudinary.v2.uploader.upload(
            pizzaPhotoFiles.large[0].path,
            options
          )
        }
        ...
        const newPizza = await prisma.pizza.create({
          data: {
            ...
            pizzaSizes: {
              create: [
                ...parsedPizzaSizes.map((pizzaSize: any) => ({
                  ...pizzaSize,
  ...
  async editPizza(pizzaId: string, pizzaData: any, pizzaPhotoFiles: any) {
    const removePhoto = () => {
      if (pizzaPhotoFiles.small) {
        fs.rmSync(
          path.join(
    ...
    try {
      ...
      const pizzaWithGivenData = await prisma.pizza.findUnique({
        where: { name: pizzaData.name },
      })
      if (!pizza) {
        removePhoto()
        ...
        if (pizzaWithGivenData) {
          ...
          if (
            pizzaWithGivenData.id !== pizza.id &&
  ...
  async changePizzaStatus(id: string, status: Status) {
    try {
