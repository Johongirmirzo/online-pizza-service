...
const CategoryService = {
      ...
      if (category) {
        if (categoryPhotoFiles) {
          fs.rmSync(
            path.join(
              __dirname,
              `../../assets/images/${categoryPhotoFiles.filename}`
            )
          )
        }
        return {
        ...
        const categoryPhoto = await cloudinary.v2.uploader.upload(
          categoryPhotoFiles.path,
          options
        )
        const newCategory = await prisma.category.create({
          data: {
            ...categoryData,
            photo: categoryPhoto.secure_url,
            photo_id: categoryPhoto.public_id,
          },
        })
  ...
  async editCategory(
    ...
    categoryPhotoFiles: any
  ) {
    try {
      const category = await prisma.category.findUnique({ where: { id } })
      const categoryWithGivenName = await prisma.category.findUnique({
        where: { name: categoryData?.name },
      })
      ...
      if (!category) {
        ...
        if (categoryPhotoFiles) {
          ...
          categoryPhoto = await cloudinary.v2.uploader.upload(
            categoryPhotoFiles.path,
            options
          )
        }
