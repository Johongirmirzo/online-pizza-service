...
const FeedbackService = {
    async getAllFeedbacks(){
        return await prisma.feedback.findMany({include: {dineInFeedbacks: true, deliveryFeedbacks: true}, orderBy: [{created: "desc"}]});
    },
    async getFeedback(id: number){
        try {
            const feedback = await prisma.feedback.findUnique({where: {id}, include: {dineInFeedbacks: true, deliveryFeedbacks: true}});
    ...
    async changeFeedbackStatus(id: number, status: FeedbackStatus){
        try {
