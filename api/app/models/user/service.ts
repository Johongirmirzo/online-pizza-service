...
const UserService = {
    ...
    async getAllUsers(){
        return (await prisma.user.findMany({orderBy: [{created: "desc"}]})).filter(user => Role[user.role]!= Role.ADMIN);
    },
    async getUser(id: number, reqObject: any){
        try {
    ...
    async createNewUser(userData: IUser, userPhotoFile: any){
        try {
            const user = await prisma.user.findFirst({where: {OR: [{email: userData.email}, {phoneNumber: userData.phoneNumber}]}})
            if(user){
                if(userPhotoFile){
                    fs.rmSync(path.join(__dirname, `../../assets/images/${userPhotoFile.filename}`))
                }
                ...
                let userPhoto;
                if(userPhotoFile){
                    userPhoto = await cloudinary.v2.uploader.upload(userPhotoFile.path, options);
                }
    ...
    async editUser(id: number, userData: IUserFormData, userPhotoFile: any){
        try {
            const userWithGivenData = await prisma.user.findFirst({where: {OR: [{email: userData.email}, {phoneNumber: userData.phoneNumber}]}})
            const user = await prisma.user.findUnique({where: {id}})
            if(!user){
                if(userPhotoFile){
                    fs.rmSync(path.join(__dirname, `../../assets/images/${userPhotoFile.filename}`))
