...
const UserController = {
    async login(req: Request, res: Response){
        try {
            ...
            const {user, statusCode, error} = await UserService.loginUser(userEmail, password);
            if(statusCode === 400 || statusCode === 404){
                ...
                if(user){
                    ...
                    const accessToken = generateToken<ITokenPayload>({id, name, email, role: Role[user.role]}, `${ACCESS_TOKEN_PRIVATE_KEY}`, `${ACCESS_TOKEN_EXPIRATION_TIME}`)
                    const refreshToken = generateToken<ITokenPayload>({id, name, email, role: Role[user.role]}, `${REFRESH_TOKEN_PRIVATE_KEY}`, `${REFRESH_TOKEN_EXPIRATION_TIME}`)
    ...
    async createUser(req: Request, res: Response){      
        const {newUser, statusCode, error} = await UserService.createNewUser(req.body, req.file)
    ...
    async editUser(req: Request, res: Response){
        const id = Number(req.params.id);
        console.log({id})
        const {user, statusCode, error} = await UserService.editUser(id, req.body, req.file);
    ...
    async changeUserStatus(req: Request, res: Response){
        const id = Number(req.params.id);
        const {statusCode, error} = await UserService.changeUserStatus(id, req.body.status);
        if(statusCode === 400 || statusCode === 404 || statusCode === 403){
            return res.status(statusCode).json({errors: error});
        }  else {
