...
export const orderHandler = (io: any, socket: any) => {
  ...
  socket.on(
    'change-order-status',
    async (orderData: { orderId: string; status: OrderStatus }) => {
      try {
  ...
  socket.on(
    ...
    async (orderData: { orderId: string; status: PaymentStatus }) => {
      try {
        await prisma.order.update({
          where: { id: orderData.orderId },
          data: {
...
router.get(
  '/get-all-customer-orders/:id',
  validateReqParamForId,
