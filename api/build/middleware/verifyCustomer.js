...
const decodeCustomerToken_1 = __importDefault(require("../utils/decodeCustomerToken"));
const reissueCustomerAccessToken_1 = __importDefault(require("../utils/reissueCustomerAccessToken"));
const verifyCustomer = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (authHeader) {
        ...
        if (accessToken) {
            const decodedToken = (0, decodeCustomerToken_1.default)(accessToken, `${process.env.ACCESS_TOKEN_PRIVATE_KEY}`);
            if (decodedToken.decoded) {
                req.customer = decodedToken.decoded;
                return next();
            }
            if (refreshToken && decodedToken.isExpired) {
                const newAccessToken = (0, reissueCustomerAccessToken_1.default)(refreshToken);
                if (newAccessToken) {
                    const decodedToken = (0, decodeCustomerToken_1.default)(newAccessToken, `${process.env.ACCESS_TOKEN_PRIVATE_KEY}`);
                    req.customer = decodedToken.decoded;
                    return next();
                }
            }
