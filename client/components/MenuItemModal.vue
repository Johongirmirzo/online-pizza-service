          ...
          <div>
            ...
            <div v-else>
              <p class="modal__menu-item-piece-amount">
                {{ menuData?.pieces[menuItemPieceIndex]?.amount }}
              </p>
...
watchEffect(() => {
  totalItemPrice.value =
    itemAmount.value *
    Number(props.menuData.pieces[menuItemPieceIndex.value].price);
});

const incrementMenuItemAmount = () => {
  if (
    totalItemPrice.value +
      Number(props.menuData.pieces[menuItemPieceIndex.value].price) >
    1200000
...
const decrementMenuItemAmount = () => {
  if (itemAmount.value > 1) {
    itemAmount.value -= 1;
    totalItemPrice.value -= Number(
      props.menuData.pieces[menuItemPieceIndex.value].price
    );
    console.log(
      totalItemPrice.value,
      Number(props.menuData.pieces[menuItemPieceIndex.value].price)
...
const isMenuItemActive = () => props.menuData.status === "ACTIVE";

const handleAddToCartBtn = () => {
  if (!isMenuItemActive()) return;
  const cartItem = {
    ...props.menuData,
    id: props.menuData.id,
    itemType: "Menu Item",
    isEditable: props.menuData.pieces.length > 1,
    activeIndex: menuItemPieceIndex.value,
    itemId: uuid(),
    vegan: props.menuData.vegan,
    photo: props.menuData.pieces[menuItemPieceIndex.value].photo,
    name: props.menuData.name,
    total: totalItemPrice.value,
    amount: itemAmount.value,
    size: props.menuData.pieces[menuItemPieceIndex.value].size,
  };
  if (cartStore.calculateTotal() + cartItem.total > 1200000) {
    ...
    $toast.success("Menu Item is added to cart!");
